...
class TestMockAdoWorkitemApi:
    ...
    def add_work_item1(self, mock_workitem_api: MockWorkitemsApi):
        work_item = WorkItem(source_id=1, type='Bug', assigned_to='John', title='Test', state="New")
        mock_workitem_api.work_items.append(work_item)
        return work_item.source_id

    @pytest.fixture
    def add_work_item2(self, mock_workitem_api):
        work_item = WorkItem(source_id=2, type='Feature', assigned_to='Jane', title='Test2', state="New")
        mock_workitem_api.work_items.append(work_item)
        return work_item.source_id
    ...
    def test_update_work_item(self, mock_workitem_api: MockWorkitemsApi, add_work_item1):
        updated_work_item = UpdateWorkItemInput(source_id=1, type='Bug', assigned_to='Jane')
    ...
    def test_update_work_item_not_found(self, mock_workitem_api):
        with pytest.raises(ValueError):
            mock_workitem_api.update_work_item(UpdateWorkItemInput(source_id=1, type='Bug', assigned_to='Jane'))
